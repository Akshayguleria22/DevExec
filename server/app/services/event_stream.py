import json
from datetime import datetime, timezone
from typing import Any
from uuid import UUID

from sqlalchemy import func
from sqlalchemy.exc import IntegrityError
from sqlalchemy.orm import Session

from app.core.redis import redis_connection
from app.models.execution_event import ExecutionEvent


def _utc_now_iso() -> str:
    return datetime.now(timezone.utc).isoformat()


def get_task_event_channel(task_id: str) -> str:
    return f"task_events:{task_id}"


def _serialize_event(event: ExecutionEvent) -> dict[str, Any]:
    return {
        "id": str(event.id),
        "task_id": str(event.task_id),
        "seq": event.seq,
        "event_type": event.event_type,
        "payload": event.payload,
        "created_at": event.created_at.isoformat() if event.created_at else _utc_now_iso(),
    }


def publish_execution_event(
    db: Session,
    task_id: UUID,
    event_type: str,
    payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    payload = payload or {}
    max_attempts = 5
    event: ExecutionEvent | None = None

    for _ in range(max_attempts):
        next_seq = (
            db.query(func.coalesce(func.max(ExecutionEvent.seq), 0))
            .filter(ExecutionEvent.task_id == task_id)
            .scalar()
            + 1
        )

        event = ExecutionEvent(
            task_id=task_id,
            seq=next_seq,
            event_type=event_type,
            payload=payload,
        )
        db.add(event)

        try:
            db.commit()
            db.refresh(event)
            break
        except IntegrityError:
            db.rollback()
            event = None
            continue

    if event is None:
        raise RuntimeError("Failed to assign deterministic sequence to execution event")

    message = _serialize_event(event)
    redis_connection.publish(get_task_event_channel(str(task_id)), json.dumps(message))
    return message


def build_transient_event(
    task_id: str | None,
    event_type: str,
    payload: dict[str, Any] | None = None,
    seq: int | None = None,
) -> dict[str, Any]:
    return {
        "id": None,
        "task_id": task_id,
        "seq": seq,
        "event_type": event_type,
        "payload": payload or {},
        "created_at": _utc_now_iso(),
    }


def get_execution_events(db: Session, task_id: UUID, limit: int = 500) -> list[dict[str, Any]]:
    rows = (
        db.query(ExecutionEvent)
        .filter(ExecutionEvent.task_id == task_id)
        .order_by(ExecutionEvent.seq.asc(), ExecutionEvent.created_at.asc())
        .limit(limit)
        .all()
    )
    return [_serialize_event(row) for row in rows]
